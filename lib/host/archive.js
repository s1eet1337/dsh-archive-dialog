 ...
 */
export function isSafeSessionId(raw) {
 ...
 */
export function snapshotHeader(entry) {
    if (entry === undefined || entry === null || typeof entry !== 'object')
        return undefined;
    const wrapped = entry.header;
...
async function loadHeaders(persistence) {
...
export function isSessionLogFile(name) {
    return SESSION_LOG_FILE.test(name);
}
function encodeSegmentSafe(sessionId) {
    try {
 ...
 */
export function scanStoredSessionDirs(sessionsRoot) {
    const found = new Map();
    let projects;
    try {
 ...
 */
export function locateSessionDirs(sessionsRoot, cwd, sessionId) {
...
export async function listArchived(registry, persistence, liveSessions, options = {}) {
 ...
 */
export async function applyQueryTitles(rows, query) {
 ...
 */
export async function listUngrouped(registry, persistence, liveSessions, options = {}) {
    ...
    if (headersLoaded)
        for (const id of headers.keys())
            candidates.add(id);
    for (const id of liveHeaders.keys())
        candidates.add(id);
    const rows = [];
    for (const sessionId of candidates) {
        if (archived.has(sessionId) || accounted.has(sessionId))
            continue;
        const header = headers.get(sessionId);
        const liveHeader = liveHeaders.get(sessionId);
        const raw = (header ?? liveHeader);
        if (raw === undefined)
            continue;
        if (String(raw.origin ?? '') === 'subagent')
            continue;
 ...
 */
export async function archiveSession(registry, sessionId) {
    const state = registry.requireState();
    const current = Array.isArray(state.archivedSessionIds) ? state.archivedSessionIds.map(String) : [];
    if (current.includes(sessionId))
        return { archived: false, archivedSessionIds: current };
 ...
 * 幂等：已在集合里就不写。
 */
async function addToArchive(registry, sessionId) {
...
export function markRecentlyDeleted(sessionId) {
...
export function isRecentlyDeleted(sessionId) {
    return recentlyDeleted.has(sessionId);
}
/** 仅测试用：忘掉「本进程删过」的记忆，让墓碑重新可被 purge 回收。 */
export function clearRecentlyDeleted() {
...
export async function purgeStaleArchived(deps) {
    ...
    let known;
    try {
        known = new Set((await deps.persistence.list())
            .map((entry) => {
            const header = snapshotHeader(entry);
 ...
 */
async function artifactStillPresent(persistence, sessionId) {
    ...
    try {
        for (const entry of await persistence.list()) {
            const header = snapshotHeader(entry);
            if (header !== undefined && String(header.id ?? header.sessionId ?? '') === sessionId)
                return true;
        }
        return false;
    }
 ...
 */
async function removeEmptySessionDir(sessionsRoot, sessionId) {
    const encoded = encodeSegmentSafe(sessionId);
    if (encoded === undefined)
        return;
    for (const dir of scanStoredSessionDirs(sessionsRoot).get(encoded) ?? []) {
...
async function removeProjectionCache(projectCacheRoot, sessionId) {
    const encoded = encodeSegmentSafe(sessionId);
    if (encoded === undefined)
        return;
 ...
 */
async function finishDelete(deps, sessionId, agent, filesDeleted) {
...
export async function deleteSession(deps, sessionId) {
    ...
    if (sessionDir !== undefined) {
        ...
        throw new Error(`未找到会话日志目录：会话日志仍存在但目录定位失败，拒绝删除（sessionsRoot=${deps.sessionsRoot}）`);
    }

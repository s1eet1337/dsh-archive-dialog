 ...
 */
import { archiveSession, deleteSession, isSafeSessionId, listArchived, listUngrouped, purgeStaleArchived, restoreSession } from "./host/archive.js";
import { projectionCacheRoot, resolveDshHome, sessionsRoot } from "./host/paths.js";
import { isTrustedApiRequest, readJsonBody, writeJson } from "./host/wire.js";
...
export function apply(ctx) {
    ...
    const readRegistry = () => ctx.get('workspaceRegistry');
    const readPersistence = () => ctx.get('sessionPersistence');
    const readSessions = () => ctx.get('sessions');
    ...
    mountRoutes();
    const disposeServiceWait = ctx.on('internal/service', (svc) => {
        if (svc === 'webServer' || svc === 'httpServer')
            mountRoutes();
    });
    if (typeof disposeServiceWait === 'function')
        ctx.effect(disposeServiceWait, 'dsh-archive-dialog: late route mount');
}

...
async function getRows(path: string): Promise<ApiResult<ArchivedRow[]>> {
...
export function getArchived(): Promise<ApiResult<ArchivedRow[]>> {
  return getRows(`${API_PREFIX}/archived`)
}

/** 「未分组」对话：未归档、也没有工作区记账的会话。 */
export function getUngrouped(): Promise<ApiResult<ArchivedRow[]>> {
  return getRows(`${API_PREFIX}/ungrouped`)
}

export async function postArchive(sessionId: string): Promise<ApiResult<unknown>> {
